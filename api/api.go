...
func New(ctx context.Context, cfg *config.Config, r chi.Router, idc *identity.Client, rsp responder, g generator, d datastore, ds datasetAPIClient, c cantabularClient, p kafka.IProducer) *API {
	...
	tracer, err := apm.NewTracer("cantabular-filter-flex", "some-version")
	if err != nil {
		println(err.Error())
		panic("Tracer err!")
	}
	...
	api.tracer = tracer
	print(api.tracer.Stats().SpansSent, "SETNTNNTS-------------------------")
...
func (api *API) enablePublicEndpoints() {
	...
	api.Router.Delete("/filters/{id}/dimensions/{name}/options", api.deleteFilterDimensionOptions)
...
func (api *API) enablePrivateEndpoints() {
	...
	r.Delete("/filters/{id}/dimensions/{name}/options", api.deleteFilterDimensionOptions)
	...
	err := http.ListenAndServe(":27100", r)
	if err != nil {
		panic(err.Error())
	}
