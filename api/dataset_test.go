...
func TestInvalidGeography(t *testing.T) {
	Convey("When geography cannot be accessed an error is returned", t, func() {
		...
		gomock.InOrder(
			...
			ctblrMock.EXPECT().GetGeographyDimensionsInBatches(p.ctx, datasetResponse.IsBasedOn.ID, optionsBatch, optionsWorker).Return(nil, expectedError).Times(1),
		)
...
func TestStaticDatasetQuery(t *testing.T) {
	Convey("When StaticDatasetQuery is called with an invalid query it returns an error", t, func() {
		...
		gomock.InOrder(
			...
			ctblrMock.EXPECT().GetGeographyDimensionsInBatches(p.ctx, datasetResponse.IsBasedOn.ID, optionsBatch, optionsWorker).Return(getValidGeoResponse(), nil).Times(1),
